...
func parseStructFields(t reflect.Type, val reflect.Value, s *schema.JSONSchema, opts *schema.Options) {
	for i := range t.NumField() {
		...
		if shouldOmitField(field, i, val, opts) {
			continue
		}
...
// shouldOmitField checks whether a field should be skipped due to OmitEmpty option.
func shouldOmitField(field reflect.StructField, idx int, val reflect.Value, opts *schema.Options) bool {
	if opts == nil || !opts.OmitEmpty || !fieldHasOmitempty(field) {
		return false
	}

	return val.IsValid() && isZeroValue(val.Field(idx))
}
...
func buildArrayDetail(elemType reflect.Type, opts *schema.Options) *schema.UISchemaElement {
	...
	buildUIElements(elemType, reflect.Value{}, "#/properties", detail, opts)
